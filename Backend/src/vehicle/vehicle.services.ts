...
export const createVehicleService = async (vehicle: any) => {
    ...
    .returning({registration_number: vehiclesTable.registration_number})
    .execute();
}
// Fetch all vehicles
...
return await db.query.vehiclesTable.findFirst({
    where : eq(vehiclesTable.registration_number, registration_number)
})
}
// update vehicle by registration number
export const updateVehicleService = async (registration_number: string, vehicle: any) => {
    return await db.update(vehiclesTable).set(vehicle).where(eq(vehiclesTable.registration_number, registration_number)).execute();
}
//delete vehicle by registration number
export const deleteVehicleService = async (registration_number: string) => {
    await db.delete(vehiclesTable).where(eq(vehiclesTable.registration_number, registration_number));
